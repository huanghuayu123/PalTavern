import type { CharacterProfile, ChatMessage, ModelMessage, PromptPreset } from './types';
import { chatStylePreset, stickerUsageContext } from './chat-format';
import { characterRelationshipContextFor } from './character-relationships';
import { characterSettingsText } from './character-settings';
import { characterStatusContextFor, characterStatusFor } from './character-status';
import { isImpactSourceRolledBack } from './impacts';
import { applyPromptPresetRegexScripts } from './prompt-presets';
import { hasModelBudget, messagesFor, recordModelRequest, saveState, state } from './state';
import { companionTimeContext } from './time';
import { timelineContextFor } from './timeline';
import { compactText, firstString, isRecord } from './utils';
import { refreshWorldWeather, shouldRefreshWorldWeather, worldWeatherPromptContext } from './weather';
...
export async function testModelConnection(input: ModelConnectionTestInput): Promise<ModelConnectionTestResult> {
  ...
  const content = firstString(
    ...
    isRecord(message) ? message.reasoning_content : undefined,
    isRecord(message) ? message.reasoning : undefined,
...
export async function callModel(
  ...
  return applyPromptPresetRegexScripts(content, regexPreset)
