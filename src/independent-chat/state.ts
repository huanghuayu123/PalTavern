...
} from './auto-message-strategy';
import {
  BUILTIN_CHARACTER_CARDS,
  type BuiltinCharacterCardDefinition,
} from './builtin-character-cards';
import { migrateInlineSettingsToWorldBook } from './character-settings';
import {
...
function normalizeDrafts(value: unknown): CharacterCardDraft[] {
  ...
  return value.filter(isRecord).map(draft => {
    const mode = draft.mode === 'complex' ? 'complex' as const : 'simple' as const;
    const allowedSteps = mode === 'complex'
      ? ['identity', 'appearance', 'hobbies', 'palette', 'reinterpretation', 'preview']
      : ['identity', 'appearance', 'personality', 'hobbies', 'preview'];
    const currentStep = typeof draft.currentStep === 'string'
      && draftSteps.includes(draft.currentStep as CharacterCardDraftStep)
      && allowedSteps.includes(draft.currentStep)
      ? draft.currentStep as CharacterCardDraftStep
      : 'identity';
...
function normalizeCharacterInteractionType(value: unknown): CharacterInteractionType {
  return value === 'world_event' || value === 'mention' ? value : 'moment_comment';
}
...
export function normalizeState(input: unknown): AppState {
  ...
  return {
    ...
    activeGroupChatId,
    activeView: parsed.activeView === 'groups'
      || parsed.activeView === 'moments'
      || parsed.activeView === 'events'
      || parsed.activeView === 'timeline'
      ? parsed.activeView
      : 'chat',
    chatReplyMode,
    enterToSend,
...
export function ensureWorldExists(worldId: string): void {
  ...
  const now = Date.now();
  state.worlds.push({ id: worldId, name: '未命名世界', description: '', userPersona: '', createdAt: now, updatedAt: now });
}
