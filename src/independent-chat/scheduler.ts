import { appendAssistantReply, setStatusText } from './chat';
import { pacingStrategyFor, pacingStyleFor } from './auto-message-strategy';
import {
...
import { generateWorldEvent } from './events';
import { recordImpact, recordTimelineEntryImpact } from './impacts';
import { callModel } from './model';
import { generateCharacterMoment, spreadMomentInteractions } from './moments';
import { sendLocalNotification } from './notifications';
import { installNativeBackgroundCheckHandler, requestNativeBackgroundSchedule } from './platform';
import {
...
import { addAutoMessageTimelineEntry } from './timeline';
import { waitForModelTyping } from './typing-delay';
import type { AutoEventSchedule, AutoMomentSchedule, CharacterProfile, PacingState, QuietHours } from './types';
...
async function attemptWorldInteraction(): Promise<void> {
  const now = Date.now();
...
async function attemptCharacterMoment(character: CharacterProfile): Promise<void> {
  const schedule = momentSchedule(character);
...
async function attemptCharacterEvent(character: CharacterProfile): Promise<void> {
  const schedule = eventSchedule(character);
...
async function attemptCharacter(character: CharacterProfile): Promise<void> {
  const schedule = character.autoMessage;
...
async function tick(onChange: () => void): Promise<void> {
  ...
  try {
    for (const character of state.characters) {
      await attemptCharacter(character);
      await attemptCharacterMoment(character);
      await attemptCharacterEvent(character);
    }
    await attemptWorldInteraction();
