...
function buildDirectReplyPrompt(
  ...
  const threadMessages = characterDirectMessagesFor(thread.id);
  const target = options.replyToId
    ? threadMessages.find(message => message.id === options.replyToId)
...
export async function runBackgroundCharacterDirectDialogue(
  ...
  thread.lastAutoGeneratedAt = messages[messages.length - 1]?.createdAt ?? createdAt;
