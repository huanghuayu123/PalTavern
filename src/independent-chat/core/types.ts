...
export interface TimelineSourceRef {
  type: 'message' | 'group_message' | 'direct_chat' | 'moment' | 'comment' | 'event' | 'relationship' | 'brief' | 'status' | 'interaction' | 'system' | 'manual';
  id: string;
}
...
export type SummaryLayer = 'micro' | 'middle' | 'macro';
export type MemorySummaryStatus = 'active' | 'paused' | 'pending_confirmation';
export type MemorySummaryScope = 'character' | 'event' | 'relationship' | 'world' | 'chapter' | 'conversation';
...
export interface CharacterDirectThread {
  id: string;
  worldId: string;
  participantCharacterIds: string[];
  lastReadByCharacterId: Record<string, number>;
  lastAutoGeneratedAt?: number;
  createdAt: number;
  updatedAt: number;
}

export interface CharacterDirectMessage {
  id: string;
  threadId: string;
  worldId: string;
  speakerCharacterId: string;
...
export type PrivateChatEventSuggestionStatus = 'pending' | 'accepted' | 'dismissed';
export type PrivateChatEventSuggestionSourceKind = 'private_chat' | 'character_direct';

export interface PrivateChatEventSuggestion {
  id: string;
  worldId: string;
  ...
  affinityDelta: number;
  reason: string;
  status: PrivateChatEventSuggestionStatus;
