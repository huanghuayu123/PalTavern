...
import type { CharacterCardDraft, CharacterCardDraftStep } from './types';
...
function stepDescription(step: CharacterCardDraftStep): string {
  const descriptions: Record<CharacterCardDraftStep, string> = {
    ...
    palette: '从底色、主色、点缀色和衍生表现逐层建立完整性格。',
...
function renderPreview(draft: CharacterCardDraft): string {
  const personality = draft.mode === 'complex'
    ? [
      draft.palette ? `【性格调色盘】\n${draft.palette}` : '',
      draft.reinterpretation ? `【二次解释】\n${draft.reinterpretation}` : '',
    ].filter(Boolean).join('\n\n')
    : draft.personality;
  return `
    ...
    <main class="authoring-screen authoring-chooser">
      ...
      <section class="authoring-choice-content">
        <div class="authoring-choice-intro">
          <span class="settings-kicker">选择创作方式</span>
          <h2>从一个想法，逐步写成可以聊天的角色</h2>
          <p>两种模式都会自动保存。模型负责提问和整理，你始终决定最终写进卡里的内容。</p>
        </div>
        <div class="authoring-mode-grid">
          <button class="authoring-mode-card" data-create-draft="simple">
            <span>简单版</span><h3>快速建立角色</h3>
            <p>角色构想 → 外貌 → 性格 → 爱好 → 预览</p>
            <strong>适合第一次写卡</strong>
          </button>
          <button class="authoring-mode-card featured" data-create-draft="complex">
            <span>复杂版</span><h3>细化人物内在逻辑</h3>
            <p>角色构想 → 外貌 → 爱好 → 性格调色盘 → 二次解释 → 预览</p>
            <strong>适合长期角色</strong>
          </button>
        </div>
      </section>
    </main>
    ...
    <main class="authoring-screen">
      <header class="authoring-topbar">
        ...
        <div class="authoring-title">
          <span>${draft.mode === 'complex' ? '复杂版' : '简单版'} · ${draft.name ? escapeHtml(draft.name) : '未命名角色'}</span>
...
export function bindAuthoringUi(rerender: () => void): void {
  ...
  document.querySelectorAll<HTMLButtonElement>('[data-create-draft]').forEach(button => {
    button.addEventListener('click', () => {
      const draft = createCharacterCardDraft(button.dataset.createDraft === 'complex' ? 'complex' : 'simple');
    ...
    <div class="draft-manager">
      ...
      ${drafts.map(draft => `
        <article class="draft-row">
          <div>
            <span>${draft.mode === 'complex' ? '复杂版' : '简单版'} · ${STEP_LABELS[draft.currentStep]}</span>
